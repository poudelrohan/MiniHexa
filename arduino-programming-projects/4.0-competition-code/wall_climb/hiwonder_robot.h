...
class Robot {
  public:
    ...
    /**
     * @brief Enable/disable ripple wave gait for slippery terrain.
     *        In wave gait only 2 legs swing at a time; 4 remain grounded.
     *        The stomp lift makes each swinging leg rise high and come
     *        straight down rather than sweep, maximising normal force.
     *
     * @param enabled      - true = wave gait, false = default tripod
     * @param stomp_height - how high each leg lifts during swing (cm-units).
     *                       0 = auto (2× the normal leg-lift value).
     */
    void set_wave_gait(bool enabled, float stomp_height = 0.0f);

    /**
     * @brief Set per-position leg lift heights for the tripod gait.
     *        Front legs (1,6), middle legs (2,5), rear legs (3,4)
     *        can each have a different lift amount.
     *
     * @param front  - lift height for front legs (legs 1, 6)
     * @param mid    - lift height for middle legs (legs 2, 5)
     * @param rear   - lift height for rear legs (legs 3, 4)
     */
    void set_leg_lifts(float front, float mid, float rear);

    /**
     * @brief Enable/disable caterpillar sequential gait.
     *        Leg pairs step in sequence, 4 legs always grounded.
     *        front_first=false (default): rear(1,6)→mid(2,5)→front(3,4) — backward climb
     *        front_first=true:            front(3,4)→mid(2,5)→rear(1,6) — forward climb
     *
     * @param enabled     - true = caterpillar, false = default tripod
     * @param lift_rear   - lift height for rear legs (1, 6)
     * @param lift_mid    - lift height for middle legs (2, 5)
     * @param lift_front  - lift height for front legs (3, 4)
     * @param front_first - true = front pair steps first (forward climb)
     */
    void set_caterpillar(bool enabled,
                         float lift_rear  = 7.0f,
                         float lift_mid   = 4.0f,
                         float lift_front = 2.0f,
                         bool  front_first = false);

    /**
     * @brief Enable/disable metachronal (1-at-a-time) gait.
     *        Only 1 leg swings at a time; 5 always grounded.
     *        Maximum stability for steep/slippery slopes.
     *        6-phase cycle: each leg gets 1/6 cycle for swing.
     *
     * @param enabled     - true = metachronal, false = default tripod
     * @param lift_rear   - lift height for rear legs (1, 6)
     * @param lift_mid    - lift height for middle legs (2, 5)
     * @param lift_front  - lift height for front legs (3, 4)
     * @param front_first - true = front legs step first (forward climb)
     */
    void set_metachronal(bool enabled,
                         float lift_rear  = 3.0f,
                         float lift_mid   = 5.0f,
                         float lift_front = 8.0f,
  ...
  private:
    ...
    int last_step_num;
    float _leg_lift = 3;
    bool  wave_gait_mode = false;   // ripple gait: 2 legs swing, 4 stable
    float _stomp_height  = 0.0f;   // 0 = auto (2× _leg_lift)
    float _lift_front    = 3.0f;   // tripod z lift for front legs (1, 6)
    float _lift_mid      = 3.0f;   // tripod z lift for middle legs (2, 5)
    float _lift_rear     = 3.0f;   // tripod z lift for rear legs (3, 4)
    bool  caterpillar_mode    = false; // sequential paired gait: rear→mid→front
    float _cat_lift_rear      = 7.0f; // caterpillar lift: rear legs (3, 4)
    float _cat_lift_mid       = 4.0f; // caterpillar lift: mid legs (2, 5)
    float _cat_lift_front     = 2.0f; // caterpillar lift: front legs (1, 6)
    bool  _cat_front_first    = false; // true = front pair steps first (forward climb)
    bool  metachronal_mode     = false; // 1-at-a-time gait: 5 legs always grounded
    float _meta_lift_rear      = 3.0f;
    float _meta_lift_mid       = 5.0f;
    float _meta_lift_front     = 8.0f;
    bool  _meta_front_first    = true;
    uint8_t act_read_frame_num = 0;
    const float max_half_step_length = 4.5f;  // increased for bigger steps
