    ...
    case CRAWL:
      ...
      switch(move_state) {
        case MOVING:
          ...
          if (metachronal_mode) {
            // ---- METACHRONAL GAIT (1 leg at a time) ----
            // Only 1 leg swings at once; 5 always grounded.
            // Maximum stability for steep/slippery slopes.
            //
            // 6-phase cycle: each leg gets 1/6 cycle to swing.
            // Front-first order: 3(front-R)→4(front-L)→2(mid-R)→5(mid-L)→1(rear-R)→6(rear-L)
            // Rear-first order:  1(rear-R)→6(rear-L)→2(mid-R)→5(mid-L)→3(front-R)→4(front-L)
            //
            // Lift profile: 15% rise / 70% hold / 15% drop (slope-optimised).

            RobotLeg* legs[6] = { &leg1, &leg2, &leg3, &leg4, &leg5, &leg6 };
            // Phase determines when each leg swings (0=first, 5=last)
            // Front-first: front pair first, then mid, then rear (alternating R/L)
            const int sp_front[6] = { 4, 2, 0, 1, 3, 5 };
            // Rear-first: rear pair first, then mid, then front
            const int sp_rear[6]  = { 0, 5, 4, 3, 2, 1 };
            const int *sp = _meta_front_first ? sp_front : sp_rear;
            const float lift[6] = { _meta_lift_rear,  _meta_lift_mid,  _meta_lift_front,
                                     _meta_lift_front, _meta_lift_mid,  _meta_lift_rear };

            float global_t = (float)tick_count / (float)move_time; // 0 → 1

            for (int i = 0; i < 6; i++) {
              RobotLeg *L = legs[i];
              // leg_t in [0, 6): 0–1 = swing, 1–6 = stance (5 sub-phases)
              float leg_t = fmod(global_t * 6.0f - (float)sp[i] + 6.0f, 6.0f);

              if (leg_t < 1.0f) {
                // SWING: lift and step
                float sw = leg_t;
                L->trajectory_point.x = L->result.x + L->amplitude.x * (2.0f * sw - 1.0f);
                L->trajectory_point.y = L->result.y + L->amplitude.y * (2.0f * sw - 1.0f);
                // Slope-optimised stomp: 15% rise / 70% hold / 15% drop
                float h;
                if      (sw < 0.15f) h = lift[i] * (sw / 0.15f);
                else if (sw < 0.85f) h = lift[i];
                else                 h = lift[i] * ((1.0f - sw) / 0.15f);
                L->trajectory_point.z = L->result.z + h;
              } else {
                // STANCE: propulsion — sweep backward
                float st = (leg_t - 1.0f) / 5.0f; // 0 → 1 over 5 sub-phases
                L->trajectory_point.x = L->result.x + L->amplitude.x * (1.0f - 2.0f * st);
                L->trajectory_point.y = L->result.y + L->amplitude.y * (1.0f - 2.0f * st);
                L->trajectory_point.z = L->result.z;
              }
            }

          } else if (caterpillar_mode) {
            // ---- CATERPILLAR SEQUENTIAL GAIT ----
            // Leg PAIRS step in strict order: rear(1,6) → mid(2,5) → front(3,4)
            // Only 2 legs swing at a time; 4 always grounded.
            //
            // Physical leg positions (Y coordinate tells front/rear):
            //   Rear  legs: 1 (Y=-5.65, R) and 6 (Y=-5.65, L)  → negative Y = rear
            //   Middle legs: 2 (Y=0, R)    and 5 (Y=0, L)
            //   Front legs: 3 (Y=+5.65, R) and 4 (Y=+5.65, L)  → positive Y = front
            //
            // Phase assignment (sp): rear=0, mid=1, front=2
            //   leg1(rear-R)=0,  leg2(mid-R)=1,  leg3(front-R)=2
            //   leg4(front-L)=2, leg5(mid-L)=1,  leg6(rear-L)=0
            //
            // Each leg gets 1/3 of the cycle to swing, 2/3 in stance.
            // Lift profile: fast rise → hold at peak → fast drop (stomp).
            // Lift height: rear=high (claw in), mid=med, front=low (trailing).

            RobotLeg* legs[6] = { &leg1, &leg2, &leg3, &leg4, &leg5, &leg6 };
            // Phase assignment depends on direction:
            //   rear-first (backward climb): rear(1,6)=0, mid(2,5)=1, front(3,4)=2
            //   front-first (forward climb): front(3,4)=0, mid(2,5)=1, rear(1,6)=2
            const int sp_rear[6]  = { 0, 1, 2, 2, 1, 0 };
            const int sp_front[6] = { 2, 1, 0, 0, 1, 2 };
            const int *sp = _cat_front_first ? sp_front : sp_rear;
            // per-leg lift heights
            const float lift[6]  = { _cat_lift_rear,  _cat_lift_mid,  _cat_lift_front,
                                      _cat_lift_front, _cat_lift_mid,  _cat_lift_rear };

            float global_t = (float)tick_count / (float)move_time; // 0 → 1

            for (int i = 0; i < 6; i++) {
              RobotLeg *L = legs[i];
              // leg_t in [0,3): 0–1 = swing, 1–3 = stance
              float leg_t = fmod(global_t * 3.0f - (float)sp[i] + 3.0f, 3.0f);

              if (leg_t < 1.0f) {
                // SWING: step backward (plant further back on slope)
                float sw = leg_t; // 0 → 1
                L->trajectory_point.x = L->result.x + L->amplitude.x * (2.0f * sw - 1.0f);
                L->trajectory_point.y = L->result.y + L->amplitude.y * (2.0f * sw - 1.0f);
                // Slope-optimised lift profile: 15% rise / 70% hold / 15% drop.
                // The robot is tilted ~30°, so the slope surface rises under the
                // foot as it sweeps backward.  Keeping the leg at peak height for
                // 70% of the swing (vs 40% in the standard stomp) guarantees the
                // foot clears the slope throughout the entire backward sweep and
                // only descends right at the end when it is ready to plant.
                float h;
                if      (sw < 0.15f) h = lift[i] * (sw / 0.15f);          // fast rise
                else if (sw < 0.85f) h = lift[i];                          // hold HIGH
                else                 h = lift[i] * ((1.0f - sw) / 0.15f); // fast drop
                L->trajectory_point.z = L->result.z + h;
              } else {
                // STANCE: propulsion — push body uphill
                float st = (leg_t - 1.0f) / 2.0f; // 0 → 1
                L->trajectory_point.x = L->result.x + L->amplitude.x * (1.0f - 2.0f * st);
                L->trajectory_point.y = L->result.y + L->amplitude.y * (1.0f - 2.0f * st);
                L->trajectory_point.z = L->result.z;
              }
          ...
          } else if (wave_gait_mode) {
            // ---- RIPPLE WAVE GAIT ----
            // Only 2 legs swing at once; 4 legs stay grounded.
            // Pairs and their swing sub-phase (0, 1, or 2):
            //   Phase 0: Leg 1 (front-R) + Leg 4 (rear-L)   — diagonal
            //   Phase 1: Leg 3 (rear-R)  + Leg 6 (front-L)  — diagonal
            //   Phase 2: Leg 2 (mid-R)   + Leg 5 (mid-L)    — sides
            //
            // Stomp trajectory: leg lifts HIGH and comes straight down
            // (30% rise / 40% hold / 30% drop) for maximum normal force
            // on the slippery crater wall.
            //
            // Amplitude reuse: the same amplitude computed by
            // cal_omni_move_end_point() is reused here.  Because each leg
            // only gets 1/3 of the cycle for swing (vs 1/2 in tripod)
            // the robot moves slightly slower per cycle — which is exactly
            // what we want for the crater.

            const float stomp = (_stomp_height > 0.0f) ? _stomp_height : (_leg_lift * 2.0f);

            // Pointer array so we can loop over legs
            RobotLeg* legs[6] = { &leg1, &leg2, &leg3, &leg4, &leg5, &leg6 };
            // swing_phase assignment: index = leg-1, value = 0/1/2
            const int sp[6] = { 0, 2, 1, 0, 2, 1 };

            float global_t = (float)tick_count / (float)move_time; // 0 → 1

            for (int i = 0; i < 6; i++) {
              RobotLeg *L = legs[i];
              // leg_t in [0, 3): 0–1 = swing window, 1–3 = stance window
              float leg_t = fmod(global_t * 3.0f - (float)sp[i] + 3.0f, 3.0f);

              if (leg_t < 1.0f) {
                // --- SWING: lift and step forward ---
                float sw = leg_t; // 0 → 1

                // Horizontal: sweep from rear (-amp) to front (+amp)
                L->trajectory_point.x = L->result.x + L->amplitude.x * (2.0f * sw - 1.0f);
                L->trajectory_point.y = L->result.y + L->amplitude.y * (2.0f * sw - 1.0f);

                // Vertical stomp profile
                float lift;
                if      (sw < 0.30f) lift = stomp * (sw / 0.30f);            // fast rise
                else if (sw < 0.70f) lift = stomp;                           // hold at peak
                else                 lift = stomp * ((1.0f - sw) / 0.30f);  // fast drop
                L->trajectory_point.z = L->result.z + lift;

              } else {
                // --- STANCE: propulsion (leg sweeps backward) ---
                float st = (leg_t - 1.0f) / 2.0f; // 0 → 1 over stance duration

                // Horizontal: from front (+amp) to rear (-amp)
                L->trajectory_point.x = L->result.x + L->amplitude.x * (1.0f - 2.0f * st);
                L->trajectory_point.y = L->result.y + L->amplitude.y * (1.0f - 2.0f * st);
                L->trajectory_point.z = L->result.z; // stay on ground
              }
            }
          } else {
            // ---- ORIGINAL TRIPOD GAIT ----
            ...
            // Per-leg lift heights:
            //   Tripod A swings first (legs 1=front-R, 3=rear-R, 5=mid-L)
            //   Tripod B swings second (legs 2=mid-R, 4=rear-L, 6=front-L)
            float cos_val = cosf(2 * PI * tick_count / move_time);
            if(tick_count > (move_time / 4) && tick_count < (3 * move_time / 4)) {
              // Tripod A grounded, Tripod B swinging
              leg1.trajectory_point.z = leg1.result.z;
              leg3.trajectory_point.z = leg3.result.z;
              leg5.trajectory_point.z = leg5.result.z;

              leg2.trajectory_point.z = leg2.result.z - _lift_mid   * cos_val;  // mid
              leg4.trajectory_point.z = leg4.result.z - _lift_rear  * cos_val;  // rear
              leg6.trajectory_point.z = leg6.result.z - _lift_front * cos_val;  // front
            } else {
              // Tripod B grounded, Tripod A swinging
              leg1.trajectory_point.z = leg1.result.z + _lift_front * cos_val;  // front
              leg3.trajectory_point.z = leg3.result.z + _lift_rear  * cos_val;  // rear
              leg5.trajectory_point.z = leg5.result.z + _lift_mid   * cos_val;  // mid
...
void Robot::set_wave_gait(bool enabled, float stomp_height) {
  wave_gait_mode = enabled;
  _stomp_height  = stomp_height;
}

void Robot::set_leg_lifts(float front, float mid, float rear) {
  _lift_front = front;
  _lift_mid   = mid;
  _lift_rear  = rear;
}

void Robot::set_caterpillar(bool enabled, float lift_rear, float lift_mid, float lift_front, bool front_first) {
  caterpillar_mode  = enabled;
  _cat_lift_rear    = lift_rear;
  _cat_lift_mid     = lift_mid;
  _cat_lift_front   = lift_front;
  _cat_front_first  = front_first;
}

void Robot::set_metachronal(bool enabled, float lift_rear, float lift_mid, float lift_front, bool front_first) {
  metachronal_mode  = enabled;
  _meta_lift_rear   = lift_rear;
  _meta_lift_mid    = lift_mid;
  _meta_lift_front  = lift_front;
  _meta_front_first = front_first;
}
