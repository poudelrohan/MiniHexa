...
class Robot {
  public:
    ...
    /**
     * @brief Action group run with speed control.
     *        Same as action_group_run() but every frame's
     *        move_time is multiplied by speed_factor.
     *
     * @param  id           - action group ID to run
     * @param  speed_factor - timing multiplier
     *                        1.0 = original speed
     *                        2.0 = twice as slow  (good for crater)
     *                        0.5 = twice as fast
     *                        Minimum clamped to 50 ms per frame.
     */
    void action_group_run_scaled(uint8_t id, float speed_factor);
