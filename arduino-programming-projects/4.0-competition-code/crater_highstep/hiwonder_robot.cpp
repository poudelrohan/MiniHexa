...
void Robot::action_group_run_scaled(uint8_t id, float speed_factor) {
  uint8_t    frame_index;
  uint8_t    control_num;
  uint8_t    buf[58] = {0}; /* frame_index|control_num|time_l|time_h|id|duty_l|duty_h|... */
  uint16_t   move_time;
  uint16_t   scaled_time;
  ServoArg_t servos[18];
  ...
  File file = SPIFFS.open("/ActionGroup" + String(id) + ".rob", FILE_READ);
  if (!file) {
    ESP_LOGI("Robot", "action_group_run_scaled: failed to open file\n");
    return;
  }
  ...
  while (file.available()) {
    switch (act_state) {
      ...
      case READ_FRAME_DATA:
        ...
        // Apply speed factor — larger = slower, smaller = faster.
        // Clamp to 50 ms minimum so servos always have time to move.
        scaled_time = (uint16_t)(move_time * speed_factor);
        if (scaled_time < 50) scaled_time = 50;

        for (uint8_t i = 0; i < control_num; i++) {
          servos[i].id   = buf[4 + i * 3];
          servos[i].duty = BYTE_TO_HW(buf[6 + i * 3], buf[5 + i * 3]);
        }
        servo.multi_set(servos, control_num, scaled_time);
        delay(scaled_time);
